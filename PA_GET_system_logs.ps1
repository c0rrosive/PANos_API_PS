...
$result_delete = Invoke-RestMethod -Uri ($URL + 'api/?type=log&key='+ $API_TOKEN +'&log-type=system&action=finish&job-id=' + $filtered_response.response.result.job);
...
do
 {
     ...
     switch ($option)
     {
         ...
         ([XML](Get-Content ($filebase + '_system_raw.xml'))).response.result.log.logs.entry | ft -auto -Property receive_time,action,src,from,dst,to,rule,app,dport,proto;
		 } 'q' {
             exit 0;
         } 'n' {
			exit 0;
